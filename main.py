...
def create_app():
    ...
    def register_city():
        ...
        if exists:
            return jsonify({"message": "Could not register the city, it already exists"}), 400
    ...
    def find_flights(fromCity, toCity):
        ...
        # Ieskome skrydziu tarp siu miestu su ne daugiau kaip 3 sustojimais
        flight_query = """
        MATCH path = (from_city:City  {name: $fromCity})-[:HAS_AIRPORT]->(from_airport:Airport)-[:HAS_FLIGHT*1..4]->
                     (f:Flight)-[GOES_TO*1..4]->(to_airport:Airport)<-[:HAS_AIRPORT]-(to_city:City {name: $toCity})
        WITH path, 
            [r IN nodes(path) WHERE 'Flight' IN labels(r) | r.number] AS flight_ids,
            reduce(total_price = 0, r IN nodes(path)| total_price + COALESCE(r.price, 0)) AS total_price,
            reduce(total_time = 0, r IN nodes(path)| total_time + COALESCE(r.flightTimeInMinutes, 0)) AS total_time,
            [n IN nodes(path) WHERE 'Airport' IN labels(n)| n.code] AS airport_codes
        RETURN path, flight_ids, airport_codes, total_price, total_time
        ORDER BY total_price ASC, total_time ASC
        """
        ...
        for flight in flights:
            results.append({
                "fromAirport": flight["airport_codes"][0],
                "toAirport": flight["airport_codes"][-1],
                "flights": flight["flight_ids"],
                "price": flight["total_price"],
                "flightTimeInMinutes": flight["total_time"]
            })
